...
async def get_list_categories(
    session: AsyncSession, category_ids: Iterable[list[int]]
) -> Sequence[Category]:
    ...
    if not res:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail={"categories": "not found!"}
        )
    return res


async def get_courses_with_single_or_few_categories(
    session: AsyncSession, category_id: int
) -> list[int]:
    query = select(CourseHasCategory.course_id).where(
...
async def get_categories_by_ids(
    session: AsyncSession, course_has_category: list[CourseHasCategory]
) -> Sequence[Category]:
    q = select(Category).where(
        Category.id.in_(c.category_id for c in course_has_category)
    )
    res = await session.execute(q)
